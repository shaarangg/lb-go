...
// Reverse proxy is an HTTP handler that takes an incoming request and forwards it to another server
type Backend struct {
	URL          *url.URL
	Alive        bool
	mux          sync.RWMutex
...
func main() {
	...
	for _, tok := range tokens {
		...
		proxy.ErrorHandler = func(w http.ResponseWriter, r *http.Request, err error) {
			...
			// if the same request routes to different backends, increate the count of attempts
			attempts := GetAttemptsFromContext(r)
